...
export const insufficientSOPMFields = (
...
export type ParameterizedModifier<ParameterSet extends ModifierParameterSet> = {
  /**
   * この{@link ParameterizedModifier}の動作を制御するパラメータ集合。
   */
   ...
   * さらに、`def2` がこの {@link ParameterizedModifier} を含んでいるとする。
   * すると、この {@link ParameterizedModifier} は `def1` の評価結果(SOPM)を参照してSOPMの変換を行おうとする可能性がある。
   * そのような場合では、当メソッドは `Set(def1.definitionUid)` を返せばよい。
   *
   * もし仮に、この {@link ParameterizedModifier} が `def3` の結果を参照しよとしていたならば、それは無効なプログラムである。
  ...
  /**
   * この{@link ParameterizedModifier}に、部分的な(後述){@link SGPEvaluationResult}と{@link SOPM}を与えて実行する。
   *
