...
describe('subsetOf', () => {
  ...
  it.each([
    new Set([1]), new Set([1, '']), new Set([''])
  ])('empty set must return false when an nonempty set is given', s => {
    expect(subsetOf(new Set())(s)).toBe(false);
  });

  it.each([
    new Set([1]), new Set([1, 2]), new Set([1, 2, 3])
  ])('set with itself must return true', s => {
  ...
  ])('set must return when containing an element not in the second set', (s1, s2) => {
