...
export function sampleDensely(
  ...
  const sufficientlyClose = (p1: SampledPoint, p2: SampledPoint) =>
    (p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2 < maximumRangeGap ** 2 ||
    Math.abs(p1.t - p2.t) < minimumDomainGap;
...
export function spreadSamplesOver(
  samples: SampledPoint[],
  interpolatedPoints: number,
  isOpenIntervalRight = false
): Point[] {
  type SampleWithDistance = SampledPoint & { distance: number };

  // p1とp2の距離を返す関数
  const distanceBetween = (p1: SampledPoint, p2: SampledPoint) =>
  ...
  const linearlyInterpolate = (
    p1: SampleWithDistance,
    p2: SampleWithDistance,
  ...
  // p_0からp_n-1の総距離
  const totalDistance =
    samplesWithDistances[samplesWithDistances.length - 1].distance;
  // 1点辺りの距離
  const distancePerSample =
    totalDistance / (interpolatedPoints - (isOpenIntervalRight ? 0 : 1) || 1);
  ...
  for (let i = 0; i < interpolatedPoints; i++) {
    ...
    while (
      sampleIndexLowerBound < samplesWithDistances.length - 2 &&
      samplesWithDistances[sampleIndexLowerBound + 1].distance < targetDistance
    ) {
